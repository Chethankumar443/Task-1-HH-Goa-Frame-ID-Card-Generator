...
export default function VoiceRAGApp() {
  ...
  const handleTextSubmit = async (customQuery?: string) => {
    ...
    try {
      ...
      setRagResult({
        query: q,
        answer:
          'Hybrid search combines dense vector retrieval (FAISS) with sparse keyword search (BM25) using Reciprocal Rank Fusion (RRF). FAISS indexes 384-dimensional embeddings generated by SentenceTransformers, enabling sub-50ms semantic matching across high-density knowledge bases [1].',
        citations: [
          {
            chunk_id: 1,
            text: 'Hybrid search combines dense vector embeddings from FAISS with sparse keyword frequency scores from BM25. A 70/30 weighted fusion provides high recall and precise keyword matching.',
  ...
  const handleVoiceSubmit = async (audioBlob: Blob) => {
    ...
    try {
      ...
      setSttStatus(null);
      const simulatedVoiceQ = 'What is hybrid vector search using FAISS and BM25?';
      setQueryInput(simulatedVoiceQ);
      setRagResult({
        query: simulatedVoiceQ,
        answer:
          'FAISS handles dense embedding similarity search using inner-product distance, while BM25 Okapi evaluates exact term frequency-inverse document frequency. Reciprocal Rank Fusion merges both score distributions [1][2].',
        citations: [
          {
            chunk_id: 1,
            text: 'Hybrid search combines dense vector embeddings from FAISS with sparse keyword frequency scores from BM25. A 70/30 weighted fusion provides high recall and precise keyword matching.',
                  ...
                  >
                    ...
                    <div className="mt-auto pt-4 border-t border-emerald-800/40 flex flex-wrap items-center justify-between gap-3 font-mono text-[10px] text-emerald-300/60">
                      <div className="flex flex-wrap items-center gap-4">
                        ...
                        <span>
                          LLM Gen <strong className="text-emerald-100">{ragResult.latency_ms.generation_ms}ms</strong>
                        </span>
                      </div>
